...
cpu_rt_rc=0
if [[ "${1:-}" == "--rt" || "${1:-}" == "-rt" ]]; then
  bash "${workspace_root}/scripts/setup_cpu_rt_host.sh" "${2:-}" || cpu_rt_rc=$?
...
if [[ -f "${workspace_root}/.pixi/rt-profile-enabled" ]]; then
  ...
  _rt_name="$(tr -d '[:space:]' < "${workspace_root}/.pixi/rt-profile" 2>/dev/null || true)"
  if [[ -z "${_rt_name}" ]]; then
    echo "  rt profile: (missing .pixi/rt-profile — run pixi run setup-rt <robot>)"
    _rt_name=""
  fi
  if [[ -n "${_rt_name}" ]]; then
    # shellcheck disable=SC1090
    source "${workspace_root}/scripts/rt_cpu_profile.${_rt_name}.env"
    echo "  rt robot:  ${_rt_name}"
    ...
    echo "  cm_aff:    ${RT_CM_CPU_AFFINITY:-none}"
