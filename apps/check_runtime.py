"""Read-only runtime check for one embodiment profile.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Read-only RMI runtime check")
...
def main() -> None:
    ...
    with rmi.Context.from_profile(args.profile) as context:
        ...
        get_diagnostics = getattr(
            context.authority_client, "get_hardware_diagnostics", None
        )
        ...
        result = {
            ...
            "allocations": context.authority_client.get_allocations(),
        }
