...
def main() -> None:
    ...
    for name, cfg in teleoperators.items():
        ...
        status = "DETECTED (Streaming)" if pubs else "NO PUBLISHER (Start workstation_stack.launch.py)"
        icon = "[✓]" if pubs else "[!]"
    ...
    try:
        ...
        with ExitStack() as stack:
            # Wire RMI Admission sessions for all configured teleoperators
            for name, cfg in teleoperators.items():
                agent = ctx.make_agent(cfg["target_agent"], frequency=cfg.get("publish_rate_hz", 200.0))
                parts = [cfg["arm_part"], cfg["gripper_part"]]
                session = stack.enter_context(agent.run(ctx.robot, parts=parts))
            ...
            while True:
                ...
                set_teleop_preempt(ctx.node, teleoperators, is_active)
                if is_active:
