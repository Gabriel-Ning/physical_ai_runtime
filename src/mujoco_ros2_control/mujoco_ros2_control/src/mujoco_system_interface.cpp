...
#endif
{
  ...
  simulation_->set_pre_step_callback([this](mjData* data) { this->pre_step_callback(data); });

#if !ROS_DISTRO_HUMBLE
  // Verify and harmonize the update rate
  ...
  if (under_sampled)
  {
    RCLCPP_INFO(
        get_logger(),
        "Aligning MuJoCo simulator timestep from %.6f sec (%lu Hz) to controller manager update rate %.6f sec (%lu Hz).",
        simulation_->model()->opt.timestep, static_cast<unsigned long>(1.0 / simulation_->model()->opt.timestep),
        desired_timestep, static_cast<unsigned long>(get_hardware_info().rw_rate));
    simulation_->model()->opt.timestep = desired_timestep;
  }
#endif
                                                   ...
                                                   const std::vector<std::string>& stop_interfaces)
{
  auto update_joint_interface = [this](const std::string& interface_name, bool enabled) {
    ...
    if (!enabled)
    {
      for (auto* actuator_it : actuators)
      {
        actuator_it->is_claimed = false;
        if (actuator_it->mj_pos_adr >= 0 && std::isfinite(control_state_.qpos[actuator_it->mj_pos_adr]))
        {
          actuator_it->position_interface.state_ = control_state_.qpos[actuator_it->mj_pos_adr];
        }
      }
                                                             ...
                                                             const rclcpp::Duration& period)
{
  ...
  {
    if (actuator.actuator_type == ActuatorType::PASSIVE || !actuator.is_claimed)
    {
      continue;
    }
...
void MujocoSystemInterface::pre_step_callback(mjData* data)
{
  const mjModel* model = simulation_->model();
  const int nv = model->nv;

  // Gravity-only compensation (FCI-like): evaluate RNE bias at zero velocity.
  // Using the full qfrc_bias also cancels Coriolis/centrifugal terms; with a
  // high-gain impedance loop that makes the effort plant non-passive and can
  // diverge as soon as the arm starts moving (hold stays stable).
  std::vector<mjtNum> qvel_saved(static_cast<size_t>(nv));
  std::vector<mjtNum> gravity(static_cast<size_t>(nv), 0.0);
  mju_copy(qvel_saved.data(), data->qvel, nv);
  mju_zero(data->qvel, nv);
  mj_rne(model, data, 0, gravity.data());
  mju_copy(data->qvel, qvel_saved.data(), nv);
  ...
  for (auto& actuator : mujoco_actuator_data_)
  {
    ...
    if (actuator.mj_vel_adr < 0)
    {
      continue;
    }

    if (actuator.is_effort_control_enabled || !actuator.is_claimed)
    {
      data->qfrc_applied[actuator.mj_vel_adr] = gravity[static_cast<size_t>(actuator.mj_vel_adr)];
    }
    else
    {
      data->qfrc_applied[actuator.mj_vel_adr] = 0.0;
    }
  }
}
...
void MujocoSystemInterface::load_mujoco_plugins()
{
  ...
  // Connect system interface and plugin pre-step callbacks directly to the physics simulation.
  simulation_->set_pre_step_callback([this](mjData* data) {
    this->pre_step_callback(data);
    for (auto& plugin : plugin_instances_)
    {
