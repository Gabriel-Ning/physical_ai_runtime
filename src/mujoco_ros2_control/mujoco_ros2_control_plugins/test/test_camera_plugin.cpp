...
// Sampling follows the supplied state even before /clock is delivered.
TEST_F(CameraPluginTest, SamplesStateTimeWithoutClockDelivery)
{
  ...
  std::atomic<int> received_sec{ -1 };
  auto sub = node_->create_subscription<sensor_msgs::msg::Image>(
      "/camera_plugin/test_cam/color", 1,
      [&](sensor_msgs::msg::Image::SharedPtr msg) { received_sec = msg->header.stamp.sec; });
  wait_for_subscriber_match(sub);
  // Also check recovery after the source state time is reset backwards.
  for (const int sample_sec : { 2, 1 })
  {
    data_->time = sample_sec;
    const auto deadline = std::chrono::steady_clock::now() + WAIT_TIMEOUT;
    while (received_sec != sample_sec && std::chrono::steady_clock::now() < deadline)
    {
      plugin.update(model_, data_);
      std::this_thread::sleep_for(POLL_INTERVAL);
    }
    EXPECT_EQ(received_sec.load(), sample_sec);
  }
  plugin.cleanup();
}
...
TEST_F(CameraPluginTest, ShmOnlyCreatesNoImageOrInfoPublishers)
{
  ...
  load_model(R"(<mujoco><worldbody>
    <geom type="plane" size="2 2 0.1"/>
    <camera name="test_cam" pos="0 0 1" resolution="64 64"/>
    <camera name="unconfigured_cam" pos="0 0 1" resolution="64 64"/>
  </worldbody></mujoco>)");
  mj_forward(model_, data_);
  mujoco_ros2_control_plugins::CameraPlugin plugin;
  EXPECT_TRUE(plugin.init(plugin_node_, model_, data_, [] { return 0; }));
  wait_for_rendering(plugin);
  EXPECT_EQ(plugin_node_->count_publishers("/camera_plugin/test_cam/color"), 0u);
  EXPECT_EQ(plugin_node_->count_publishers("/camera_plugin/test_cam/depth"), 0u);
  EXPECT_EQ(plugin_node_->count_publishers("/camera_plugin/test_cam/camera_info"), 0u);
  EXPECT_FALSE(std::filesystem::exists("/dev/shm" + shm_prefix + "unconfigured_cam"));
  mujoco_ros2_control_plugins::CameraShmReader reader;
  std::string error;
  EXPECT_TRUE(reader.open(shm_prefix + "test_cam", error)) << error;
  for (int frame = 1; frame <= 3; ++frame)
  {
    ...
    {
      plugin.update(model_, data_);
      if (reader.read_latest(nullptr, nullptr, sec, nsec)) { break; }
      std::this_thread::sleep_for(POLL_INTERVAL);
    }
    EXPECT_EQ(sec, frame);
  ...
  EXPECT_FALSE(std::filesystem::exists("/dev/shm" + shm_prefix + "test_cam"));
}
