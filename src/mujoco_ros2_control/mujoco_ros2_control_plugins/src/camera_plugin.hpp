...
/**
 * @brief Where rendered frames are delivered.
 *
 * ROS keeps the historical in-process Image publishers. SHM writes a seqlock ring for an
 * out-of-process bridge (gz-bridge style). BOTH does both for debugging.
 */
...
struct CameraData
{
  ...
  // Optional POSIX SHM ring for out-of-process ROS Image publishing.
  std::string shm_name;
  int shm_fd{ -1 };
  void* shm_map{ nullptr };
  std::size_t shm_map_size{ 0 };
  CameraShmStatic* shm_static{ nullptr };
  uint64_t shm_sequence{ 0 };
};
...
private:
  ...
  bool init_camera_shm(CameraData& camera);
  void close_camera_shm(CameraData& camera);
  void write_camera_shm(CameraData& camera, const rclcpp::Time& stamp);
  ...
  CameraOutput output_mode_{ CameraOutput::ROS };
  std::string shm_prefix_{ "/pai_mj_cam_" };
