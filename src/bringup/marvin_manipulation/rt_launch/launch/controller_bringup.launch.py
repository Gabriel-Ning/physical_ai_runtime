...
def _controller_override_yaml(*, load_pika_hardware: bool) -> str | None:
    ...
    handle, path = tempfile.mkstemp(
        suffix=".yaml", prefix="marvin_controller_overrides_"
    )
    os.close(handle)
    with open(path, "w", encoding="utf-8") as stream:
        yaml.safe_dump(overrides, stream, sort_keys=False)
    return path
...
def _controller_nodes(context: LaunchContext):
    ...
    if controller_override_yaml is not None:
        controller_parameters.append(controller_override_yaml)
