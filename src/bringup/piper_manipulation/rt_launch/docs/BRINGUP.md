...
| Provider route | Piper controller | Input |
| --- | --- | --- |
...
JSPC `trajectory_behavior.max_points: 1` is a controller-side capability. A provider may
accept a multi-point `joint_reference`; how a controller consumes those points
is not an Execution Manager responsibility.
...
pins `ros2_control_node` to `RT_CM_CPU_AFFINITY` from the RT host profile.
