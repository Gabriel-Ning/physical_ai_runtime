"""No-ROS, no-motion smoke test of a loaded LeRobot policy bundle.

Complements ``compatibility.py``: that checks the Profile <-> checkpoint
contract statically, this proves the loaded weights actually produce one
finite action of the right shape.
"""
...
from .features import make_dataset_features
...
def synthetic_dry_run(
    ...
    *,
    ...
    if values.shape != (layout.action_dimension,):
        raise ValueError(
            f"dry-run action shape {values.shape} != profile "
            f"({layout.action_dimension},)"
        )
    if not finite:
