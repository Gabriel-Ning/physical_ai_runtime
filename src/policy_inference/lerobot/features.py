"""Dataset / processor feature builders for LeRobot schemas."""
...
def make_observation_features(
    layout: PolicyLayout, *, use_video: bool = False
) -> dict[str, dict]:
    """Observation-only dataset features (RTC ``hw_features``)."""
    from lerobot.utils.constants import OBS_STR
    from lerobot.utils.feature_utils import hw_to_dataset_features

    return hw_to_dataset_features(
        _hardware_observations(layout), OBS_STR, use_video=use_video
    )
...
def make_dataset_features(
    ...
    return {
        **make_observation_features(layout, use_video=use_video),
