...
- package-private, renamed OSQP, OsqpEigen, and QDLDL runtime libraries;
...
[dependencies]
ros-jazzy-manipulation-position-controllers = "==0.3.7"
```
...
The recipe builds against conda-forge `libosqp`, `osqp-eigen`, and `libqdldl`,
then bundles all three shared libraries under package-private names. This is
required because MoveIt brings in `ros-jazzy-osqp-vendor`; that package and
conda-forge `libosqp` both install `lib/libosqp.so` but provide incompatible ABIs. The
