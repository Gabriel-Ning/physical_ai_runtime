...
CallbackReturn JointSpaceImpedanceController::configure_gravity_compensation()
{
  ...
  if (urdf_xml.empty()) {
    RCLCPP_ERROR(
      get_node()->get_logger(),
      "with_gravity_compensation is true, but no robot_description is available.");
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  if (configure_gravity_compensation() != CallbackReturn::SUCCESS) {
    return CallbackReturn::ERROR;
  }
  ...
  const rclcpp::Duration & period)
{
  ...
  if (with_gravity_compensation_) {
    for (size_t i = 0; i < joint_count; ++i) {
      dynamics_q_[dynamics_q_indices_[i]] =
        state_interfaces_[2 * i].get_optional().value_or(position_target_[i]);
    }
    const auto & gravity = pinocchio::computeGeneralizedGravity(
      *dynamics_model_, *dynamics_data_, dynamics_q_);
    for (size_t i = 0; i < joint_count; ++i) {
