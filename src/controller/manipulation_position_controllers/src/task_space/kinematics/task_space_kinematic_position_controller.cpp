...
CallbackReturn TaskSpaceKinematicPositionController::on_init()
{
  ...
  auto_declare<double>("twist_zero_linear_tolerance_m_s", 1.0e-6);
  auto_declare<double>("twist_zero_angular_tolerance_rad_s", 1.0e-6);
  ...
  auto_declare<std::vector<double>>("solver.neutral_positions", {});
...
CallbackReturn TaskSpaceKinematicPositionController::load_parameters()
{
  ...
  twist_zero_linear_tolerance_m_s_ =
    get_node()->get_parameter("twist_zero_linear_tolerance_m_s").as_double();
  twist_zero_angular_tolerance_rad_s_ =
    get_node()->get_parameter("twist_zero_angular_tolerance_rad_s").as_double();
  ...
  solver_config_.neutral_positions =
    get_node()->get_parameter("solver.neutral_positions").as_double_array();
...
CallbackReturn TaskSpaceKinematicPositionController::validate_configuration() const
{
  ...
  if (!std::isfinite(twist_zero_linear_tolerance_m_s_) ||
      twist_zero_linear_tolerance_m_s_ < 0.0 ||
      !std::isfinite(twist_zero_angular_tolerance_rad_s_) ||
      twist_zero_angular_tolerance_rad_s_ < 0.0) {
    RCLCPP_ERROR(
      get_node()->get_logger(),
      "twist zero tolerances must be finite and nonnegative.");
    return CallbackReturn::ERROR;
  }
  ...
  if (!solver_config_.neutral_positions.empty() &&
    solver_config_.neutral_positions.size() != joint_names_.size())
  {
    RCLCPP_ERROR(
      get_node()->get_logger(), "solver.neutral_positions size must match joints size (%zu).",
      joint_names_.size());
    return CallbackReturn::ERROR;
  }
  if (!solver_config_.neutral_positions.empty() &&
    !common::all_finite(solver_config_.neutral_positions))
  {
    RCLCPP_ERROR(
      get_node()->get_logger(), "solver.neutral_positions entries must be finite.");
    return CallbackReturn::ERROR;
  }
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  const bool fresh_traj = has_chunk && common::reference_is_fresh(
    time.seconds(), chunk->receive_time_s, stale_timeout_s_);
  ...
  const bool fresh_twist = has_twist && common::reference_is_fresh(
    time.seconds(), twist->stamp, twist_stale_timeout_s_);
  ...
  const bool explicit_zero_twist =
    !fresh_traj && fresh_twist &&
    last_twist_linear_norm_ <= twist_zero_linear_tolerance_m_s_ &&
    last_twist_angular_norm_ <= twist_zero_angular_tolerance_rad_s_;
  if (explicit_zero_twist) {
    active_input_mode_ = 2;
    if (!twist_zero_hold_active_) {
      // Capture measured state only on the nonzero -> zero edge. command_ can
      // be ahead while tracking an integrated Cartesian target, so retaining
      // it here would keep the robot moving after the operator releases the
      // stick. Do not continuously copy q_current_: nonzero Twist must still
      // integrate from the previous command instead of waiting for feedback.
      for (size_t i = 0; i < joint_count; ++i) {
        command_[i] = q_current_[i];
        hold_position_[i] = q_current_[i];
      }
      solver_->reset(command_);
      integrated_twist_pose_valid_ = false;
      (void)seed_integrated_twist_pose(command_);
      tracking_resync_active_ = false;
      last_command_tracking_error_rad_ = 0.0;
      twist_zero_hold_active_ = true;
    }
    for (size_t i = 0; i < joint_count; ++i) {
      (void)command_interfaces_[i].set_value(hold_position_[i]);
    }
    ...
    publish_command_state(time);
    publish_status(time, /*state=*/1);  // ACTIVE, explicit zero-velocity hold
    return controller_interface::return_type::OK;
  }
