...
CallbackReturn TaskSpaceJointImpedanceController::on_init()
{
  ...
  auto_declare<bool>("with_gravity_compensation", false);
...
CallbackReturn TaskSpaceJointImpedanceController::load_parameters()
{
  ...
  with_gravity_compensation_ =
    get_node()->get_parameter("with_gravity_compensation").as_bool();
...
CallbackReturn TaskSpaceJointImpedanceController::configure_gravity_compensation(
  const std::string & urdf_xml)
{
  dynamics_model_.reset();
  dynamics_data_.reset();
  dynamics_q_indices_.clear();
  dynamics_v_indices_.clear();
  gravity_torque_.assign(joint_names_.size(), 0.0);
  ...
  try {
    dynamics_model_ = std::make_unique<pinocchio::Model>();
    pinocchio::urdf::buildModelFromXML(urdf_xml, *dynamics_model_);
    dynamics_data_ = std::make_unique<pinocchio::Data>(*dynamics_model_);
    dynamics_q_ = pinocchio::neutral(*dynamics_model_);

    for (const auto & joint_name : joint_names_) {
      if (!dynamics_model_->existJointName(joint_name)) {
        RCLCPP_ERROR(
          get_node()->get_logger(), "Joint '%s' is not present in robot_description.",
          joint_name.c_str());
        return CallbackReturn::ERROR;
      }
      const auto joint_id = dynamics_model_->getJointId(joint_name);
      const auto & joint = dynamics_model_->joints[joint_id];
      if (joint.nq() != 1 || joint.nv() != 1) {
        RCLCPP_ERROR(
          get_node()->get_logger(),
          "Joint '%s' must have nq=1 and nv=1 for gravity compensation.",
          joint_name.c_str());
        return CallbackReturn::ERROR;
      }
      dynamics_q_indices_.push_back(static_cast<int>(joint.idx_q()));
      dynamics_v_indices_.push_back(static_cast<int>(joint.idx_v()));
    }

    // Warm up Pinocchio outside the realtime update path.
    (void)pinocchio::computeGeneralizedGravity(
      *dynamics_model_, *dynamics_data_, dynamics_q_);
  } catch (const std::exception & exception) {
    RCLCPP_ERROR(
      get_node()->get_logger(), "Failed to configure gravity compensation: %s",
      exception.what());
    return CallbackReturn::ERROR;
  }
  return CallbackReturn::SUCCESS;
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  if (configure_gravity_compensation(urdf_xml) != CallbackReturn::SUCCESS) {
    return CallbackReturn::ERROR;
  }
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  if (with_gravity_compensation_ && dynamics_model_) {
    for (size_t i = 0; i < joint_count; ++i) {
      dynamics_q_[dynamics_q_indices_[i]] = q_des_[i];
    }
    const auto & gravity = pinocchio::computeGeneralizedGravity(
      *dynamics_model_, *dynamics_data_, dynamics_q_);
    for (size_t i = 0; i < joint_count; ++i) {
      effort_command_[i] = gravity[dynamics_v_indices_[i]];
    }
  }
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  if (with_gravity_compensation_) {
    for (size_t i = 0; i < joint_count; ++i) {
      dynamics_q_[dynamics_q_indices_[i]] = q_meas_[i];
    }
    const auto & gravity = pinocchio::computeGeneralizedGravity(
      *dynamics_model_, *dynamics_data_, dynamics_q_);
    for (size_t i = 0; i < joint_count; ++i) {
      gravity_torque_[i] = gravity[dynamics_v_indices_[i]];
    }
  }
