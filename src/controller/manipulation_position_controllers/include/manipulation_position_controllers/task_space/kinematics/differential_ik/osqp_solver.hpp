...
//   Cost:
//     min   0.5 · x'·P·x  +  q'·x
//     P = blkdiag( 2·J'·J + 2·λ²I + 2·w·N,  ρ·I,  ρ·I )
//     q = [ -2·J'·(v_desired·dt) - 2·w·dq_neutral;  0;  0 ]
//
//     v_desired = K · error   (gain applied to error — same as DLS)
//     ρ = slack penalty (large — slack is used only when necessary)
//     N = Z·Z' is the exact Jacobian null-space projector from SVD
//     dq_neutral = dt·N·(q_neutral - q)
//
...
public:
    ...
    const SolverConfig & config) override
  {
    ...
    if (!config_.neutral_positions.empty() &&
      config_.neutral_positions.size() != n_joints_)
    {
      last_error_ = "neutral_positions size must match joint_names size";
      return false;
    }
    if (!common::all_finite(config_.neutral_positions)) {
      last_error_ = "neutral_positions entries must be finite";
      return false;
    }
    ...
    q_neutral_ = neutral_q_;
    has_neutral_positions_ = !config_.neutral_positions.empty();
    if (has_neutral_positions_) {
      for (int i = 0; i < n; ++i) {
        const int qi = joint_q_index_[i];
        q_neutral_[qi] = config_.neutral_positions[static_cast<size_t>(i)];
      }
    }
    ...
    q_vec_.resize(n);
    q_vec_.setZero();
    nullspace_projector_.resize(n, n);
    nullspace_projector_.setZero();
    neutral_error_.resize(n);
    neutral_error_.setZero();
    neutral_increment_.resize(n);
    neutral_increment_.setZero();

    // Prime the dynamic-size SVD workspace during non-realtime configure so
    // subsequent compute() calls can reuse its storage.
    nullspace_svd_.compute(J_act_, Eigen::ComputeFullV);
    ...
    double period) override
  {
    ...
    q_vec_.noalias() = -2.0 * J_act_.transpose() * (v_desired * dt);

    // Fixed-neutral posture objective, restricted to the instantaneous exact
    // Jacobian null space:
    //   w * ||N*dq - dt*N*(q_neutral-q)||^2
    // where N=Z*Z' and columns of Z span null(J). This lets the redundant arm
    // approach its configured neutral pose without directly competing with the
    // first-order end-effector task. Joint/CBF constraints can still limit the
    // achievable null-space step.
    if (has_neutral_positions_ && config_.posture_weight > 0.0) {
      nullspace_svd_.compute(J_act_, Eigen::ComputeFullV);
      const auto & singular_values = nullspace_svd_.singularValues();
      const double sigma_max = singular_values.size() > 0 ? singular_values[0] : 0.0;
      const double rank_threshold =
        std::max(1.0, sigma_max) * kNullspaceRankTolerance;
      int rank = 0;
      for (Eigen::Index i = 0; i < singular_values.size(); ++i) {
        if (singular_values[i] > rank_threshold) {
          ++rank;
        }
      }
      ...
      nullspace_projector_.setZero();
      if (rank < n) {
        const auto Z = nullspace_svd_.matrixV().rightCols(n - rank);
        nullspace_projector_.noalias() = Z * Z.transpose();

        for (int i = 0; i < n; ++i) {
          const int qi = joint_q_index_[i];
          neutral_error_[i] = q_neutral_[qi] - q_[qi];
        }
        neutral_increment_.noalias() = dt * nullspace_projector_ * neutral_error_;
        P_dq_dense_.noalias() +=
          2.0 * config_.posture_weight * nullspace_projector_;
        q_vec_.noalias() -=
          2.0 * config_.posture_weight * neutral_increment_;
      }
    }
...
private:
  static constexpr double kNullspaceRankTolerance = 1.0e-8;
  ...
  Eigen::JacobiSVD<Eigen::MatrixXd> nullspace_svd_;
  Eigen::MatrixXd nullspace_projector_;
  Eigen::VectorXd neutral_error_;
  Eigen::VectorXd neutral_increment_;
  ...
  bool has_neutral_positions_{false};
  SolveResult result_;
};
