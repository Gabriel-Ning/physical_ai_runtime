...
///   τ = Kp (q_ref − q) + Kd (q̇_ref − dq_filt) [+ g(q)] [+ torque clamps]
...
///
/// Gravity compensation is optional and disabled by default because the target
/// Franka hardware stack already supplies it.
...
private:
  ...
  CallbackReturn configure_gravity_compensation();
  ...
  bool with_gravity_compensation_{false};

  std::unique_ptr<pinocchio::Model> dynamics_model_;
  std::unique_ptr<pinocchio::Data> dynamics_data_;
  Eigen::VectorXd dynamics_q_;
  std::vector<int> dynamics_q_indices_;
  std::vector<int> dynamics_v_indices_;
