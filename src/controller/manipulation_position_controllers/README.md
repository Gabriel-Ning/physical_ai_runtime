...
Controller motion, reference freshness, trajectory sampling, Twist
integration, and rate limiting use the `time` and `period` supplied by
`controller_manager`. Set `use_sim_time: true` on the controller manager and
all stamped command producers when a simulation backend publishes `/clock`.
With `use_sim_time: false`, the real-hardware controller manager remains on its
normal clock.

A paused simulation produces no time progress, so commands and stale timeouts
do not advance. A backwards clock jump (for example a world reset) invalidates
pre-reset references and puts the controller into measured-state hold. Zero
stamps remain rejected by default; wait for a nonzero `/clock` before enabling
command producers.
...
```text
tau = Kp (q_ref - q) + Kd (qdot_ref - dq_filt) [+ g(q)]
```
...
defaults to hardware-provided (e.g. libfranka); set
`with_gravity_compensation: true` for a torque-transparent interface to add
the Pinocchio `g(q)` term. Coriolis feed-forward is deferred.
...
| `solver.neutral_positions` | `[]` | OSQP null-space neutral pose in `joints` order. Empty disables it |
