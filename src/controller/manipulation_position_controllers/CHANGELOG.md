...
- Bundle QDLDL under the controller-private `libmpc_qdldl.so` SONAME and make
  the private OSQP library depend on that name, preventing runtime resolution
  to an ABI-incompatible public `libqdldl.so`.

## [0.3.6] - 2026-09-04

### Fixed

- Seed Pinocchio gravity compensation onto the effort command at
  `on_activate()` for JSIC and TSJIC. Previously the first cycles started at
  τ=0 and the torque-rate limiter ramped gravity in, so a MuJoCo effort arm
  sagged immediately after controller activation.
- Re-seed the measured-state hold only on the stale-hold entry edge, so a
  continuing timeout does not keep rewriting the impedance reference every
  cycle.
...
- Add optional Pinocchio gravity compensation to the joint-space and
  task-space joint impedance controllers. It defaults to disabled for Franka
  hardware stacks that already compensate gravity.

## [0.3.4] - 2026-09-01

### Added

- Add an OSQP null-space neutral-pose objective for redundant manipulators.

### Fixed

- Bundle OSQP and OsqpEigen under controller-private library names to avoid
  ABI collisions with workspace dependencies.
- Reject references timestamped even slightly ahead of the controller clock,
  removing the previous one-control-period future-skew allowance.

## [0.3.3] - 2026-08-30

### Fixed

- Make reference freshness robust to simulation clock resets: references from
  a pre-reset or future clock domain now enter measured-state hold instead of
  remaining fresh until `/clock` catches up.

## [0.3.2] - 2026-08-27

### Fixed

- Treat a fresh near-zero Twist as an explicit velocity stop in the task-space
  kinematic position controller. The controller captures measured joint state
  once on the moving-to-zero edge instead of continuing toward the previously
  integrated Cartesian target, while nonzero Twist still integrates from the
  previous command to avoid feedback lag.

### Added

- Add independently configurable linear and angular zero-Twist tolerances:
  `twist_zero_linear_tolerance_m_s` and
  `twist_zero_angular_tolerance_rad_s`.
