...
// Two co-located, coaxial joints have an exact one-dimensional null space:
// only q1 + q2 affects the tip pose, while [dq1, dq2] = [a, -a] does not.
const char * kRedundantUrdf =
  R"(<?xml version="1.0"?>
<robot name="redundant_test_arm">
  <link name="base_link"/>
  <joint name="j1" type="revolute">
    <parent link="base_link"/><child link="link1"/>
    <origin xyz="0 0 0" rpy="0 0 0"/><axis xyz="0 0 1"/>
  ...
  <link name="link2"/>
  <joint name="to_tip" type="fixed">
    <parent link="link2"/><child link="tip_link"/>
    <origin xyz="0.5 0 0" rpy="0 0 0"/>
  </joint>
  <link name="tip_link"/>
</robot>)";

const std::vector<std::string> kRedundantJoints = {"j1", "j2"};
}  // namespace
...
TEST(OsqpSolver, FixedNeutralPosePullsOnlyThroughJacobianNullspace)
{
  ts::SolverConfig config;
  config.posture_weight = 10.0;
  config.neutral_positions = {0.8, -0.8};
  config.max_joint_velocity_rad_s = 5.0;

  ts::OsqpIkSolver solver;
  ASSERT_TRUE(
    solver.configure(
      kRedundantUrdf, kRedundantJoints, "base_link", "tip_link", config));

  const std::vector<double> q = {0.2, -0.3};
  solver.reset(q);
  std::array<double, 7> target{};
  ASSERT_TRUE(solver.compute_tip_pose(q, target));

  const ts::SolveResult result = solver.solve(q, target, 0.01);
  ASSERT_TRUE(result.success);

  double neutral_error_change = 0.0;
  for (size_t i = 0; i < q.size(); ++i) {
    const double dq = result.q_command[i] - q[i];
    neutral_error_change += (q[i] - config.neutral_positions[i]) * dq;
  }
  EXPECT_LT(neutral_error_change, 0.0);

  std::array<double, 7> moved_pose{};
  ASSERT_TRUE(solver.compute_tip_pose(result.q_command, moved_pose));
  EXPECT_NEAR(moved_pose[0], target[0], 1.0e-4);
  EXPECT_NEAR(moved_pose[1], target[1], 1.0e-4);
  EXPECT_NEAR(moved_pose[2], target[2], 1.0e-4);
  EXPECT_NEAR(
    result.q_command[0] + result.q_command[1], q[0] + q[1], 1.0e-5);
}

TEST(OsqpSolver, ResetDoesNotReplaceConfiguredNeutralPose)
{
  ts::SolverConfig config;
  config.posture_weight = 10.0;
  config.neutral_positions = {0.8, -0.8};
  ...
  const ts::SolveResult result = solver.solve(q, target, 0.01);
  ASSERT_TRUE(result.success);
  double command_change = 0.0;
  for (size_t i = 0; i < q.size(); ++i) {
    command_change += std::abs(result.q_command[i] - q[i]);
  }
  EXPECT_GT(command_change, 1.0e-8);
}

TEST(OsqpSolver, RejectsNeutralPoseWithWrongSize)
{
  ts::SolverConfig config;
  config.neutral_positions = {0.0};
