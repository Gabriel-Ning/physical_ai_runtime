...
double compute_impedance_torque(
  ...
  double prev_tau, double max_tau_change, double gravity_torque = 0.0)
{
  const double pos_err = q_ref - q_meas;
  const double vel_err = qdot_ref - dq_filt;
  double tau = kp * pos_err + kd * vel_err + gravity_torque;
...
TEST(JointSpaceImpedanceControllerTest, GravityIsAddedBeforeTorqueLimits)
{
  EXPECT_DOUBLE_EQ(
    compute_impedance_torque(1.0, 1.0, 0.0, 0.0, 500.0, 30.0,
      87.0, 0.0, 1000.0, 12.0),
    12.0);
  EXPECT_DOUBLE_EQ(
    compute_impedance_torque(1.0, 1.0, 0.0, 0.0, 500.0, 30.0,
      10.0, 0.0, 1000.0, 12.0),
    10.0);
}
