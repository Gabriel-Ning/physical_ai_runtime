...
**Not spawned:** `franka_robot_state_broadcaster`. Planning and EM only need
...
EM and `rmi.Context` subscribe to `/joint_states`.
...
Command endpoints are under `/execution/<group>/`. `/execution_manager/status`
and `/execution_manager/events` are EM telemetry only (ADR 2), not command
inputs.
