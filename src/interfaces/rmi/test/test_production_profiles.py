...
def test_dual_piper_native_gripper_profile_matches_controller_contracts():
    ...
    for side, provider in (
        ...
        assert endpoints == {
            f"{side}_arm": (f"/action_sources/piper_leader_{side}/arm/joint_reference"),
            f"{side}_gripper": (
...
def test_piper_leader_configs_match_teleop_provider_ingress():
    ...
    for side, provider in (
        ...
        leader = yaml.safe_load(
            (teleop_config_dir / f"piper_leader_{side}.yaml").read_text(
                encoding="utf-8"
            )
        )["piper_leader"]["ros__parameters"]
        sources = {
            source["part"]: source["topic"]
            for source in config.execution["sources"]
            if source["provider"] == provider
        }
        assert leader["joint_reference_topic"] == sources[f"{side}_arm"]
        assert leader["gripper_reference_topic"] == sources[f"{side}_gripper"]
